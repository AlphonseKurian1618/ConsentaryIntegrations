...
the connector must pass every acceptance scenario without a Claude Skill, Anthropic SDK or API
...
- the complete scripted no-skill matrix succeeds; and
...
Do not publish a companion skill during the pilot. If procedural corrections exceed 10% despite
server validation, a later `Consentary Vault Workflows` skill may repeat sequencing and examples as
instructions only. It must contain no scripts, credentials, authorization logic, token handling, or
alternative path around phone consent. Server-side validation remains mandatory.
...
- a captured `tools/list` matching [`../../contracts/claude/tool-catalog.json`](../../contracts/claude/tool-catalog.json);
- Streamable HTTP, RFC 9728, Auth0 CIMD, PKCE, refresh, disconnect, and reconnect evidence; and
- the sanitized seven-day pilot matrix and confirmation that the connector passed without a skill.
...
Submit the connector independently. A future skill is a separate artifact; connector installation
and safe operation must never depend on it.
